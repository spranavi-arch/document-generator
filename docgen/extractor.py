...
from docgen.prompts import (
    build_extract_section_prompt,
    build_split_document_into_sections_chunk_prompt,
...
# Max output tokens (model limit) — chunked extraction keeps each response under this
...
def split_document_into_sections(doc: str, sections: list[dict]) -> list[str]:
    ...
    (a few sections per LLM call) so the full document content is returned without truncation.
    sections: list of {"name": str, "purpose": str} in reading order.
    Returns list of verbatim text strings, one per section (no overlap, no repetition, no gaps).
    """
    ...
    while start < n:
        ...
        prompt = build_split_document_into_sections_chunk_prompt(doc, sections, start, end)
        for attempt in range(2):  # retry once on JSON failure
            try:
                response = llm.generate(prompt, max_tokens=MAX_EXTRACTION_TOKENS, json_mode=True)
                ...
                if isinstance(data, dict):
                    ...
                    if isinstance(raw, list):
                        for i, t in enumerate(raw):
                            if start + i < n:
                                result[start + i] = _clean_extracted(t)
                break
            except ValueError:
                if attempt == 1:
                    # Leave result[start:end] as "" and continue so pipeline doesn't crash
                    pass
                continue
        start = end
    return result
...
def extract_sections_from_docs(doc1: str, doc2: str, sections: list[dict]) -> list[str]:
    ...
    Ensures no overlap, no repetition, no missed content within each document.
    """
    n = len(sections)
    parts1 = split_document_into_sections(doc1 or "", sections)
    parts2 = split_document_into_sections(doc2 or "", sections)
    if len(parts1) < n:
...
def extract_section_from_docs(doc1: str, doc2: str, section_name: str) -> str:
    """Extract the section text from both sample documents and combine for use as sample_text (legacy per-section)."""
    part1 = extract_one_section(doc1 or "", section_name)
    part2 = extract_one_section(doc2 or "", section_name)
...
def extract_one_section(doc: str, section_name: str) -> str:
    ...
    if not (doc or "").strip():
        return ""
    prompt = build_extract_section_prompt(doc, section_name)
    response = llm.generate(prompt, max_tokens=4096, json_mode=True)
    data = extract_json_from_llm(response)
