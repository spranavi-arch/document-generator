...
def generate_questions_for_fields(field_names: list[str]) -> dict[str, str]:
    """
    Given snake_case field names (e.g. date_of_accident, plaintiff_name), return
    a dict mapping each field to one explicit, unambiguous question to ask a case Q&A API.
    """
    if not field_names:
        return {}
...
Generate one such question for each field. No generic or vague questions."""
    ...
    for f in field_names:
        ...
        if isinstance(q, str) and q.strip():
            result[f] = _ensure_unambiguous(f, q.strip())
        else:
            result[f] = _fallback_question(f)
    return result
...
def _ensure_unambiguous(field_name: str, question: str) -> str:
    ...
    # Generic patterns that are NOT allowed (question must be more specific)
    generic = (
...
def _fallback_question(field_name: str) -> str:
    """Produce an explicit, unambiguous question from a snake_case field name when LLM omits or returns a vague one."""
    if not field_name or not field_name.strip():
        return "What is the value for this field in the case?"
    f = field_name.strip()
    lower = f.lower().replace("_", " ")
    # Build explicit questions from common patterns so there is zero ambiguity
    ...
    # Default: spell out the field so it's clear what we're asking
    words = f.replace("_", " ")
