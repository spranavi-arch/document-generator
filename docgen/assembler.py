...
class Assembler:
    """
    Joins generated sections in blueprint order and renumbers list items
    continuously. Supports sections as dict (name -> text) or list (ordered texts).
    """
    ...
    def assemble(self, blueprint: dict, sections: dict | list) -> str:
        """
...
def renumber_numbered_paragraphs(text: str) -> str:
    return Assembler.renumber_numbered_paragraphs(text)


def assemble(blueprint: dict, sections: dict | list) -> str:
    """Backward-compatible: delegates to Assembler().assemble."""
    return Assembler().assemble(blueprint, sections)
