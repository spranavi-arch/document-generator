...
"""
    ...
    # Legacy prompt
    @staticmethod
    def build_draft_review_prompt(draft_text: str, field_values: dict, category_of_document: str) -> str:
        """
        Prompt for the DocumentReviewer agent.
        Review the full draft for consistency, flow, and redundancy while preserving legal structure.
        """
        field_block = ""
        if field_values:
            field_block = "\n".join([f"- {k}: {v}" for k, v in field_values.items()])
        
        return f"""You are a Senior Legal Editor reviewing a draft document generated by junior associates.
Your task is to refine the draft for COHERENCE, CONSISTENCY, and FLOW, without altering the core legal structure or facts.

DOCUMENT CATEGORY: {category_of_document}
...
INSTRUCTIONS:
1. **Consistency Check**: Ensure party names, dates, case numbers, and defined terms are identical throughout the document. If "Plaintiff" is defined as "John Doe", do not refer to him as "Mr. Doe" unless that is standard legal style for this document type.
2. **Remove Redundancy**: If multiple sections repeat the same introductory text (e.g., "COMES NOW the Plaintiff...") in a way that breaks the flow, consolidate or smooth the transitions.
3. **Fix Transitions**: Ensure the document flows logically from one section to the next.
4. **Preserve Structure**: Do NOT change the section headings, the order of sections, or the legal arguments.
5. **Preserve Placeholders**: If a value was missing and is marked as [Field Name], KEEP IT. Do not invent data.

OUTPUT:
Return the SINGLE, POLISHED, COMPLETE document text. Do not output markdown code blocks (```), just the text.
"""
    @staticmethod
