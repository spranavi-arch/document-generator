...
class Sectioner:
    """
    Identifies logical sections (name + purpose) from two documents via LLM.
    Section text extraction is done separately (chunked) so content is not truncated.
    """
    ...
    def divide_into_sections(self, doc1: str, doc2: str) -> dict:
        ...
        for item in raw_list:
            s = self._section_item_to_dict(item)
            if s and s["name"]:
                sections.append(s)
...
def divide_into_sections(doc1: str, doc2: str) -> dict:
    """Backward-compatible function: delegates to Sectioner().divide_into_sections."""
    return Sectioner().divide_into_sections(doc1, doc2)
