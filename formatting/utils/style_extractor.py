...
def _apply_default_spacing_and_indent(style_map: dict, style_formatting: dict) -> dict:
    """Preserve template spacing and indent. Do NOT inject 0.0 when missing — leave None so
    extracted values from the template are not overwritten and the style guide doesn't show '0.0pt' everywhere."""
    return dict(style_formatting)
...
def _format_spec_to_lines(style_name: str, fmt: dict) -> list[str]:
    ...
    # Run/font (include color so output matches sample)
    run_parts = []
    if rf.get("bold"):
    ...
    if rf.get("color_rgb_hex"):
        run_parts.append(f"color: #{rf['color_rgb_hex']}")
    if run_parts:
        lines.append("- " + ", ".join(run_parts))
    # Paragraph — only show spacing/indent when present and non-zero so prompts aren't full of "0.0pt"
    para_parts = []
    if pf.get("alignment"):
        para_parts.append(f"align: {pf['alignment'].lower()}")
    sb, sa = pf.get("space_before"), pf.get("space_after")
    if sb is not None and (isinstance(sb, (int, float)) and sb != 0):
        para_parts.append(f"space before: {sb}pt")
    if sa is not None and (isinstance(sa, (int, float)) and sa != 0):
        para_parts.append(f"space after: {sa}pt")
    li, fli = pf.get("left_indent"), pf.get("first_line_indent")
    if li is not None and (isinstance(li, (int, float)) and li != 0):
        para_parts.append(f"left indent: {li}pt")
    if fli is not None and (isinstance(fli, (int, float)) and fli != 0):
        para_parts.append(f"first line indent: {fli}pt")
    ls = pf.get("line_spacing")
    ls_rule = pf.get("line_spacing_rule")
    if ls is not None and isinstance(ls, (int, float)) and ls != 0:
        if ls_rule in ("MULTIPLE", "SINGLE", "ONE_POINT_FIVE", "DOUBLE", "TRIPLE"):
            para_parts.append(f"line spacing: {ls}x")
        else:
            para_parts.append(f"line spacing: {ls}pt")
    if para_parts:
...
def extract_paragraph_format_sources(doc: Document, max_paragraphs: int = 1000) -> list[dict]:
    """
    Extract the exact formatting of each paragraph in the template, in document order.
    Returns a list of dicts: [ {"style": str, "paragraph_format": dict, "run_format": dict}, ... ].
    Use this to apply template formatting by index: output block i gets format from template paragraph i.
    This preserves spacing, indent, font, color, numbering (via style) exactly as in the template.
    """
    sources = []
    for i, (para, _tid, _r, _c) in enumerate(iter_body_blocks(doc)):
        ...
        # If no run (empty para), get run format from style so we don't lose font/color
        if not run_format and style_name:
            from_def = _format_from_style_definition(doc, style_name)
            if from_def:
                run_format = from_def.get("run_format") or {}
        sources.append({
            "style": style_name,
...
def extract_styles(doc: Document) -> dict:
    ...
    # Prevent template alignment/italic/blue poisoning: body styles get no inherited center, italic, or link blue
    ...
    for style_name in list(style_formatting.keys()):
        if (style_name or "").lower() in body_style_names:
            ...
            rf["italic"] = False
            rf["color_rgb_hex"] = "000000"
            style_formatting[style_name] = {**fmt, "paragraph_format": pf, "run_format": rf}
    # Force all styles to black and no italic in run_format so no blue/hyperlink/italic leaks anywhere
    for style_name in list(style_formatting.keys()):
        fmt = style_formatting[style_name]
        rf = (fmt.get("run_format") or {}).copy()
        rf["color_rgb_hex"] = "000000"
        rf["italic"] = False
        style_formatting[style_name] = {**fmt, "run_format": rf}
    line_samples = _extract_line_samples(doc)
