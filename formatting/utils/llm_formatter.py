...
- Summons structure (e.g. Superior Court / Supreme Court summons with two-column layout): Output blocks in the same order as the template. Left column: court (SUPERIOR COURT or SUPREME COURT OF THE STATE OF NEW YORK), county (NEW HAVEN COUNTY / COUNTY OF KINGS), parties (Plaintiff, -against-, Defendant), horizontal line, TO THE ABOVE NAMED DEFENDANT:, venue, attorneys for plaintiff, addresses, TO: defendant. Right column: Date Filed:, plaintiff name (e.g. KELLEY SKAARVA, or Index No. on same line), Index No.:, case number, **SUMMONS** (bold, document title), **Jury Trial Demanded** (bold), then summons paragraph and attorney name block. Use block_type line with the exact separator from the template (dashed line ending with X or \"-X\"). In the summons paragraph (\"You are hereby summoned to answer the complaint...\"), bold the word \"complaint\" each time: **complaint**. When the template underlines \"TO THE ABOVE NAMED DEFENDANT:\", use __TO THE ABOVE NAMED DEFENDANT:__. Bold: **SUPERIOR COURT**, **TO THE ABOVE NAMED DEFENDANT:**, **AS AND FOR A FIRST CAUSE OF ACTION:**, **NEGLIGENCE**.
...
- VERIFIED COMPLAINT caption: Right-aligned caption blocks (in order) -> Index No.: (with colon), Date Filed:, **VERIFIED**, **COMPLAINT**, **Jury Trial Demanded**. Use the template's caption/heading style; bold VERIFIED, COMPLAINT, and Jury Trial Demanded. Separator lines: use the exact line from the template (e.g. dashed line ending with X or \"-X\").
- Preamble: One block for the opening paragraph (e.g. \"Plaintiff by his attorneys, COHAN LAW FIRM PLLC, as and for a cause of action alleges, upon information and belief, as follows:\").
- Cause of action: Output section_underline (or line), then \"AS AND FOR A FIRST CAUSE OF ACTION:\" (bold; when the template shows it in italic too, use *\*\*AS AND FOR A FIRST CAUSE OF ACTION:\*\**), then \"NEGLIGENCE\" (bold), then section_underline, then each numbered allegation as a separate block with the template's list/numbered style (do not add \"1.\", \"2.\" in text). Match the template's alignment for these headings (centered or left-aligned).
...
- SUMMONS AND VERIFIED COMPLAINT cover/footer page: Output blocks in template order. Caption right column may have \"**SUMMONS AND VERIFIED**\" then \"**COMPLAINT**\" (two lines). After caption, double solid lines (template may use border); centered \"**SUMMONS AND VERIFIED COMPLAINT**\"; firm name \"**COHAN LAW FIRM PLLC**\"; *Attorneys for Plaintiff*; address/phone; certification (22 NYCRR 130-1.1(c)); signature block (right-aligned); \"Service of a copy... admitted\"; \"Dated,\" and \"Attorney(s) for ___\"; **PLEASE TAKE NOTICE:**; then \"[ ] **NOTICE OF ENTRY**\" and \"[ ] **NOTICE OF SETTLEMENT**\" with their indented text as separate blocks; \"Dated,\" \"Yours, etc.\" and \"**COHAN LAW FIRM PLLC**\" (right-aligned). Use [ ] for empty checkboxes.
- ATTORNEY'S VERIFICATION page: First block **ATTORNEY'S VERIFICATION** (centered). Then affirmation paragraph (\"MICHAEL COHAN... affirms\"); representation paragraph (indented); centered \"**SUMMONS AND VERIFIED COMPLAINT**\"; verification body; reason paragraph; \"DATED: New York, New York\" and \"November ___, 2022\"; then signature block (attorney name, **MICHAEL COHAN, ESQ.**) right-aligned. Match template alignment and indentation.
...
def _call_openai(
    ...
    # Prefer single combined block when present (avoids duplicate prompt-building passes)
